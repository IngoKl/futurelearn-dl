...
def getCourseWeekStepPage(course_id, week_id, step_id, week_num, page_title):
    ...
    for DOWNLOAD_TYPE in DOWNLOAD_TYPES:
        ...
        URLS[DOWNLOAD_TYPE] = \
            downloadURLsInPage(course_id, week_id, step_id,
                               week_num, content, DOWNLOAD_TYPE, page_title)
...
def downloadURLToFile(url, file, DOWNLOAD_TYPE):
    ...
    if not(OVERWRITE_NONEMPTY_FILES) and os.path.exists(file):
        ...
        if statinfo.st_size != 0:
            debug(
                2, "Skipping non-zero size file <{}> of {} bytes".format(file, statinfo.st_size))
            return
    ...
    response = session.get(url, headers=headers)
    if response.status_code != 200:
        print("downloadURLToFile: Failed to download url <{}> => {}".format(
            url, response.status_code))
        return

    # showResponse(response)
    debug(1, "type={}, content.len={}".format(
        DOWNLOAD_TYPE, len(response.content)))
...
def getCourseWeekPage(course_id, week_id):
    ...
    # TODO: Make this page parsing more robust: should be checking 'list
    # steps' is part of the '<ol'
    ...
    while MATCH in current[pos:]:
        ...
        if section_pos == -1:
            ...
            debug(4, "section_pos[200]=" +
                  current[section_pos:section_pos + 200])
        ...
        if title_pos == -1:
            ...
            link_title = current[title_pos:end_title_pos].replace(
                "/", ".").replace("?", "").replace("!", "")
...
def getCoursePage(course_id):
    ...
    saveDebugItem('course.' + course_id + '.response.content',
                  "'course page'", content)
    ...
    if (len(weeks_seen) <= 0):
        fatal(
            "getCoursePage: No week entries found - check your courseid and the courserun")
...
COURSE_URL = 'https://www.futurelearn.com/courses/{}/{}/todo'.format(
    course_id, course_run)
COURSE_STEP_URL = 'https://www.futurelearn.com/courses/{}/{}/steps'.format(
    course_id, course_run)
...
# REQUEST methods:
# dir(request)=['__class__', '__delattr__', '__dict__', '__dir__',
# '__doc__', '__eq__', '__format__', '__ge__', '__getattribute__',
# '__gt__', '__hash__', '__init__', '__le__', '__lt__', '__module__',
# '__ne__', '__new__', '__reduce__', '__reduce_ex__', '__repr__',
# '__setattr__', '__sizeof__', '__str__', '__subclasshook__',
# '__weakref__', '_cookies', '_encode_files', '_encode_params', 'body',
# 'copy', 'deregister_hook', 'headers', 'hooks', 'method', 'path_url',
# 'prepare', 'prepare_auth', 'prepare_body', 'prepare_content_length',
# 'prepare_cookies', 'prepare_headers', 'prepare_hooks', 'prepare_method',
# 'prepare_url', 'register_hook', 'url']
